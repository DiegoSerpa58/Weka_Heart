...
public class ZeroRConsoleService {
    ...
    private static final Logger log = LoggerFactory.getLogger(ZeroRConsoleService.class);
    ...
    public ZeroRConsoleService(DatasetLoader datasetLoader,
                               DiabetesInstanceBuilder instanceBuilder) {
        this.instanceBuilder = instanceBuilder;
        try {
            this.dataset = datasetLoader.loadFromClasspath(DATASET_PATH);
            this.model   = new ZeroR();
            this.model.buildClassifier(dataset);
            log.info("ZeroRConsoleService: ZeroR model trained successfully.");
        } catch (Exception e) {
            throw new RuntimeException("Failed to initialize ZeroRConsoleService: " + e.getMessage(), e);
        }
    }

    /**
     * Predicts the diabetes class for a single patient.
     */
    public String predict(PatientDataRequest request) throws Exception {
        double result = model.classifyInstance(instanceBuilder.build(request, dataset));
        return dataset.classAttribute().value((int) result);
    }

    /**
     * Evaluates ZeroR using k-fold cross-validation.
     *
     * @return formatted evaluation summary string
     */
    public String evaluateCrossValidation(int folds, int seed) throws Exception {
        ...
        return formatEvaluation(eval);
    }

    /**
     * Evaluates ZeroR using a percentage split.
     *
     * @return formatted evaluation summary string
     */
    ...
    private String formatEvaluation(Evaluation eval) throws Exception {
